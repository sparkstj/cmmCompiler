...
void initialIO(){
    FuncTable read = malloc(sizeof(FuncNode));
    char * read_id = "read";
    read->name = malloc(strlen(read_id)+1);
    strcpy(read->name, read_id);
    read->returnType = malloc(sizeof(TypeNode));
    read->returnType->kind = BASIC;
    read->returnType->u.basic = 0;
    read->isDefined = 1; // is Defined
    read->next = FuncHead; // add to FuncTable List
    read->VarList = NULL;
    FuncHead = read; 

    FuncTable write = malloc(sizeof(FuncNode));
    char * write_id = "write";
    write->name = malloc(strlen(write_id)+1);
    strcpy(write->name, write_id);
    write->returnType = malloc(sizeof(TypeNode));
    write->returnType->kind = BASIC;
    write->returnType->u.basic = 0;
    write->isDefined = 1; // is Defined
    write->next = FuncHead; // add to FuncTable List
    write->VarList = malloc(sizeof(Field));
    write->VarList->type = malloc(sizeof(TypeNode));
    write->VarList->type->kind = BASIC; write->VarList->type->u.basic = 0;
    write->VarList->tail = NULL;
    FuncHead = write; 
}
