...
InterCodes translate_FunDec(TreeNode root){
    ...
    func->data = malloc(sizeof(InterCodeNode));
    func->data->kind = FUNC;
...
InterCodes translate_Exp(TreeNode Exp, Operand place){
    ...
    if (numofChild == 1) {
        ...
        if (temp1->type == 1) {// ID LP Args RP
            ...
            /*while (code) {
                code->next = NULL; code->prev = NULL;
                addInterCodes(&code1, code);
                printf("code reverse %d\n", code);
                if (code == code3) break;
                code = code->prev;
            }*/
            addInterCodes(&code1, code3);
            InterCodes code4;
            code4 = malloc(sizeof(InterCodesNode));
...
InterCodes translate_CompSt(TreeNode root){
    ...
    TreeNode DefList = root->child->sibling;
    TreeNode Def;
    while (DefList) {
